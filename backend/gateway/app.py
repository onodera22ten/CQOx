...
ENGINE_URL = os.getenv("ENGINE_URL") or (Path("./").resolve().joinpath(".engine_url").read_text().strip() if Path(".engine_url").exists() else "http://localhost:8080")
...
@app.get("/")
async def root():
    return {"message": "CQOx Gateway API", "status": "running", "docs": "/docs"}
...
async def analyze_comprehensive(request: Dict[str, Any]) -> JSONResponse:
    logger.info(f"[analyze_comprehensive] Received request: {json.dumps(request, indent=2)}")
    dataset_id = request.get("dataset_id")
    if not dataset_id:
        logger.error(f"[analyze_comprehensive] Missing dataset_id in request. Keys present: {list(request.keys())}")
        raise HTTPException(422, f"dataset_id is required. Received keys: {list(request.keys())}")
    reg = _load_registry()
    item = reg.get(dataset_id)
    if not item:
        raise HTTPException(404, f"Dataset not found: {dataset_id}")
...
# Proxy scenario endpoints to engine
@app.api_route("/api/scenario/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
async def scenario_proxy(request: Request, path: str):
    """
    Proxy all /api/scenario/* requests to the engine service
    """
    try:
        # Forward the request to engine
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            url = f"{ENGINE_URL}/api/scenario/{path}"
            ...
            # Pass through all responses including errors (proxy behavior)
            return JSONResponse(content=response.json(), status_code=response.status_code)

    except httpx.RequestError as e:
        logger.error(f"Scenario proxy connection error: {str(e)}")
        raise HTTPException(503, f"Engine service unavailable: {str(e)}")
    except Exception as e:
        logger.error(f"Scenario proxy unexpected error: {str(e)}")
        raise HTTPException(500, f"Internal error: {str(e)}")


# Proxy visualization endpoints to engine
@app.api_route("/api/visualizations/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
async def visualizations_proxy(request: Request, path: str):
    """
    Proxy all /api/visualizations/* requests to the engine service
    (Includes 3D visualizations and DAG visualization)
    """
    try:
        # Forward the request to engine
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            # Make request to engine
            url = f"{ENGINE_URL}/api/visualizations/{path}"
        ...
        logger.error(f"Visualizations proxy connection error: {str(e)}")
        raise HTTPException(503, f"Engine service unavailable: {str(e)}")
    except Exception as e:
        logger.error(f"Visualizations proxy unexpected error: {str(e)}")
        raise HTTPException(500, f"Internal error: {str(e)}")


# Proxy marketing ROI endpoints to engine
@app.api_route("/api/marketing/{path:path}", methods=["GET", "POST", "PUT", "DELETE"])
async def marketing_proxy(request: Request, path: str):
    """
    Proxy all /api/marketing/* requests to the engine service
    (Includes ROI optimization, attribution, LTV prediction)
    """
    try:
        # Forward the request to engine
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            # Make request to engine
            url = f"{ENGINE_URL}/api/marketing/{path}"
        ...
        # Only catch network/connection errors, not HTTP errors
        logger.error(f"Marketing proxy connection error: {str(e)}")
        raise HTTPException(503, f"Engine service unavailable: {str(e)}")
    except Exception as e:
        logger.error(f"Marketing proxy unexpected error: {str(e)}")
        raise HTTPException(500, f"Internal error: {str(e)}")
