...
async def run_scenario(req: ScenarioRunRequest):
    ...
    try:
        ...
        elif req.scenario.startswith("{"):
            # Inline JSON dict
            scenario_spec = json.loads(req.scenario) if isinstance(req.scenario, str) else req.scenario
            scenario_id = scenario_spec.get("id", "unknown")
        else:
            # Just scenario ID - load from config/scenarios/
            scenario_id = req.scenario
            yaml_path = Path(f"config/scenarios/{scenario_id}.yaml")
            if yaml_path.exists():
                scenario_spec = parse_scenario_yaml(str(yaml_path)).dict()
            else:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"Scenario file not found: {yaml_path}"
                )
        ...
        # Generate figures for counterfactual comparison
        figures = {}
        job_id = f"scenario_{scenario_id}_{uuid.uuid4().hex[:8]}"
        figures_dir = Path("reports/figures") / job_id
        ...
        try:
            ...
            # Generate matplotlib figures for S0/S1 comparison
            matplotlib_figures = generate_all(
                df,
                mapping,
                figures_dir,
                gates=quality_gates_result,  # Pass entire quality gates result
                cas_scores={}
            )

            # Convert local paths to HTTP URLs
            for key, local_path in matplotlib_figures.items():
                path_obj = Path(local_path)
                figures[key] = f"/reports/figures/{job_id}/{path_obj.name}"

        except Exception as e:
            print(f"[scenario] Figure generation failed: {e}")
            import traceback
            traceback.print_exc()
        ...
        print(f"[scenario] Error in run_scenario: {e}")
