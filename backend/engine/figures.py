...
def fig_quality_gates_board(gates: Dict[str,Any], out: Path) -> Optional[str]:
    # Handle new quality gates structure
    if not gates:
        return None

    # Extract gates list from quality_gates_result structure
    gates_list = gates.get("gates", []) if isinstance(gates, dict) else []
    overall = gates.get("overall", {}) if isinstance(gates, dict) else {}
    ...
    # Create text summary
    lines = ["Quality Gates Summary", "=" * 30]
    for gate in gates_list[:10]:  # Show first 10 gates
        status = gate.get("status", "NA")
        metric = gate.get("metric", "unknown")
        lines.append(f"{metric:>25}: {status}")

    lines.append("=" * 30)
    lines.append(f"Pass Rate: {overall.get('pass_rate', 0)*100:.1f}%")
    lines.append(f"Decision: {overall.get('decision', 'UNKNOWN')}")
