...
from backend.engine.quality_gates_enhanced import QualityGateEvaluator
...
class CounterfactualAutomation:
    ...
    def __init__(
        ...
        self.estimator = IntegratedEstimator()
        self.visualizer = IntegratedWolframVisualizer(wolfram_path)
        self.ope_simulator = OPESimulator()
    ...
    def run_full_comparison(
        ...
        s0_result = self._run_s0_estimation(df, mapping, estimator_method)

        # Step 2: Run S1 (Counterfactual) simulation
        s1_result = self._run_s1_simulation(
            df, mapping, scenario_spec, ope_method
        )
    ...
    def _run_s0_estimation(
        ...
        result = self.estimator.run_estimator(df, mapping, method)
    ...
    def _run_s1_simulation(
        ...
        # Use OPE simulator
        sim_result = self.ope_simulator.simulate_scenario(
            df=df,
            mapping=mapping,
...
def automate_counterfactual_comparison(
    ...
    """
    automation = CounterfactualAutomation(wolfram_path=wolfram_path)
